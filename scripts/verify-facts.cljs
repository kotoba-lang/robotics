...
;;
;; THIS REGISTER'S HOSTS ANSWER 404 FOR A PATH THAT DOES NOT EXIST.
;;
;; Measured 2026-09-09: both www.mhlw.go.jp and www.jisc.go.jp answer HTTP 404
;; with a real Japanese "this page does not exist" body at the requested URL.
;; Neither serves a WAF challenge (no awswaf markers in any body). Both
;; redirect http -> https, which is what makes the unexpected-redirect
;; self-test discriminate. The missing-status is re-measured on every run.
;;
;; ISO org (iso.org) is deliberately NOT cited: it answers the automated
;; client a Cloudflare challenge (HTTP 403, "Just a moment..."). A page behind
;; that cannot be distinguished from an absent one, so it is not cited; the
;; coverage entity says not-cited, not absent.
;;
;; THE CHALLENGE DETECTOR IS STILL LOAD-BEARING. A verifier that cites a 2xx
;; page MUST tell a real page from a bot-challenge interstitial regardless of
;; who serves one -- a host could begin answering a challenge tomorrow. The
;; detector is asserted against a real excerpt of the AWS WAF challenge body
;; (the fixture below), and is not weakened even though neither current host
;; engages it today.
;;
...
;;
;; BOTH HOSTS ANSWER 404, NOT 403, FOR A PATH THAT DOES NOT EXIST.
;; Each serves a real Japanese "this page does not exist" body at the requested
;; URL. 404 is not 2xx so status still discriminates. The status each host
;; actually returned is pinned in :host/missing-status and compared on every
;; run.
;;

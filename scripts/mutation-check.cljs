...
;; The e-Gov law API throttles a tight loop and the go.jp hosts are sensitive to
;; request bursts. A mutation suite is exactly that kind of tight loop, so the
;; network half is paced. The network mutations therefore run against a MINIMAL subset
...
(defn- tmpfile [suffix]
  (path/join (os/tmpdir) (str "robotics-mutation-" suffix ".edn")))
...
(def laws-only
  ...
  (subset #{"law.industrial-safety-health"}))
...
(def with-pages
  ...
  branches."
  (subset #{"host.www-mhlw-go-jp" "host.www-jisc-go-jp"
            "law.industrial-safety-health" "mhlw.industrial-robot-notice" "jisc.home" "mhlw.workplace-safety"}))
...
(def structural
  [{:id "coverage-entry-count"
   ...
   {:id "law-entry-without-law-id"
    ...
    :mutate #(alter-entity % "law.industrial-safety-health" (fn [e] (dissoc e :egov/law-id)))
    ...
    :mutate (fn [data]
              (mapv (fn [e]
                      (if (and (= :page-identity (:source/verify e))
                               (not= "mhlw.industrial-robot-notice" (:source/id e)))
...
(def network
  [{:id "nonexistent-law" :on :laws-only
    ...
    :mutate #(alter-entity % "law.industrial-safety-health" (fn [e] (assoc e :egov/law-id "999ZZ9999999999")))
    :want-exit 1 :want-reason "nonexistent-law"}

   {:id "law-title-drift" :on :laws-only
    :why "a real id recorded under the wrong title"
    :mutate #(alter-entity % "law.industrial-safety-health" (fn [e] (assoc e :egov/law-title "労働安全衛生法（偽）")))
    :want-exit 1 :want-reason "law-title-drift"}

   {:id "law-num-drift" :on :laws-only
    :why "a real id recorded under the wrong law number"
    :mutate #(alter-entity % "law.industrial-safety-health" (fn [e] (assoc e :egov/law-num "昭和四十七年法律第五十八号")))
   ...
   {:id "fail-outranks-refusal" :on :laws-only
    ...
    :mutate (fn [d]
              (recount
                (-> d
                    (alter-entity "law.industrial-safety-health"
                                  (fn [e] (assoc e :egov/law-title "労働安全衛生法（偽）")))
                    (into UNROUTABLE-HOST))))
    :want-exit 1 :want-reason "law-title-drift"}

   {:id "page-title-drift"
    :why "a real page recorded under a title it does not carry"
    :mutate #(alter-entity % "jisc.home" (fn [e] (assoc e :page/title "JISC 日本産業標準調査会（偽）")))
    :want-exit 1 :want-reason "title-drift"}

   {:id "missing-text"
    :why "a string the page does not contain"
    :mutate #(alter-entity % "mhlw.workplace-safety"
